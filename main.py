...
#wandb.init(project="yolo", name="detect")#wandb change according to yours


cudnn.benchmark = True

rpi = "true"#input("Rpi or not ?")
...
class VideoTracker(object):
    ...
    def image_track(self, im0):
        ...
        if det is not None and len(det):  # det: (#obj, 6)  x1 y1 x2 y2 conf cls
            ...
            centra_values = bbox_xywh.cpu().numpy()  # Bounding box coordinates
            print(xyxy2xywh(det).cpu().numpy())
            if list(xyxy2xywh(det).cpu().numpy()):
               centra_values = xyxy2xywh(det[:, :4]).cpu().numpy()
               x=centra_values[0][0]
               y=centra_values[0][1]
               w=centra_values[0][2]
               h=centra_values[0][3]
               print("x =", x)  # Output: x = 577.0
...
#rpi
            if rpi=="true":
               data = {'center_x': center_x,'center_y': center_y}
